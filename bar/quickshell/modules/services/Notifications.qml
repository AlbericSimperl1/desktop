// quickshell/modules/services/Notifications.qml
pragma Singleton
import QtQuick
import Quickshell
...
QtObject {
    ...
    // ... (rest van je bestaande Notifications.qml code) ...
    ...
    function dismissToast(hash) {
        for (let i = 0; i < root.activeToasts.count; i++) {
            if (root.activeToasts.get(i).hash === hash) {
    ...
    property NotificationServer server: NotificationServer {
        ...
        onNotification: notification => {
            ...
            if ((notification.appName && notification.appName.toLowerCase().includes(_lsms)) || (notification.summary && notification.summary.toLowerCase().includes(_lsms))) {
                notification.dismiss();
                return;
            }
